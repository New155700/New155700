...
// 1. ประกาศ Type แค่ที่เดียวตรงนี้
...
export default function UnifiedPage() {
  ...
  // 2. แปลง Type ของ user ให้รองรับ role
        ...
        <div className="flex gap-4">
          {!session ? (
            ...
            <div className="flex items-center gap-4">
              {user?.role === "OWNER" && (
                <button onClick={() => setView("admin")} className="text-purple-400 font-bold border border-purple-400 px-3 py-1 rounded-lg flex items-center gap-1">
                  <ShieldCheck size={18}/> หลังบ้าน
                </button>
              )}
              <button onClick={() => signOut()} className="text-red-500 bg-red-500/10 p-2 rounded-full">
                <LogOut size={18}/>
              </button>
            </div>
          )}
